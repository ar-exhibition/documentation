...
\begin{document}
  ...
  \subsubsection{Dynamisches Laden von Inhalten}
  ...
  um 66\% eingebrochen ist, nachdem eine App um 147MB gewachsen ist.

  Ein weiterer Vorteil dieser Trennung ist die Möglichkeit die Inhalte jederzeit auszutauschen und erweitern zu können.
  Dies ist möglich ohne eine neue Version der App zu veröffentlichen und ohne ein Update auf seiten der Benutzer.
  ...
  Über die API (https://cms.hotpot.codes/api/content) kann eine Liste aller Assets mit einem Link (AssetLink) abgerufen werden, der die Inhalte der Datenbank bereitstellt.
  ...
  \subsubsection{AR Funktionalitäten}

  Wir nutzen das Unity Package \textit{AR Foundation}, um die AR Funktionalität zu ermöglichen.
  Dieses Package ist ein Wrapper zu den nativen Implementation von Apple (ARKit) und Google (ARCore).
  Es werden also gleichzeitig iOS und Android unterstützt.
  Jedoch gab es zum Zeitpunkt der Erstellung der App keine Möglichkeit auf Android Geräten eine AR Session zu speichern und wiederherzustellen.
  Dies hat zur Folge, dass nur iOS Geräte verwendet werden können.
  Trotzdem haben wir uns für ein universelles Framework entschieden,
  damit in der Zukunft schnell die Unterstützung für Android angeboten werden kann, wenn die besagte Funktion auch auf Android möglich wird.
